...
def pretraitement(dfD, dfI, dfR, state=None):
	...
	# Selection d'un pays donné
	if state is not None:
		dfD = dfD[ dfD['Country/Region'] == state ].copy(deep=True)
		dfI = dfI[ dfI['Country/Region'] == state ].copy(deep=True)
		dfR = dfR[ dfR['Country/Region'] == state ].copy(deep=True)
